...
typedef unsigned short vector_unsigned_short __attribute__ (( vector_size(8*sizeof(unsigned short)) ));
...
i40e_rxq_rearm(struct i40e_rx_queue *rxq)
{
	...
	if (rte_mempool_get_bulk(rxq->mp,
		...
		if (rxq->rxrearm_nb + RTE_I40E_RXQ_REARM_THRESH >=
			...
			for (i = 0; i < RTE_I40E_DESCS_PER_LOOP; i++) {
				...
				vec_xstd2(dma_addr0, 0,( unsigned long long *)&rxdp[i].read);
				//vec_store_len(dma_addr0, (unsigned long long *)&rxdp[i].read, 16);
				//vec_st(dma_addr0, 0,
				//       (vector_unsigned_long_long *)&rxdp[i].read);
			}
		}
	...
	for (i = 0; i < RTE_I40E_RXQ_REARM_THRESH; i += 2, rxep += 2) {
		...
		*(uint64_t *)p1 = rxq->mbuf_initializer;
		...
		//vaddr0 = vec_ld(0, (vector_unsigned_long_long *)&mb0->buf_addr);
		//vaddr1 = vec_ld(0, (vector_unsigned_long_long *)&mb1->buf_addr);
		...
		//vec_store_len(dma_addr0, (unsigned long long *)&rxdp++->read, 16);
		//vec_store_len(dma_addr1, (unsigned long long *)&rxdp++->read, 16);
		//vec_st(dma_addr0, 0, (vector_unsigned_long_long *)&rxdp++->read);
		//vec_st(dma_addr1, 0, (vector_unsigned_long_long *)&rxdp++->read);
	}
		   ...
		   uint16_t nb_pkts, uint8_t *split_packet)
{
	...
	for (pos = 0, nb_pkts_recd = 0; pos < nb_pkts;
		...
		*(vector_unsigned_long_long *)&rx_pkts[pos + 2] =  mbp2;
		...
		pkt_mb4 = (vector_unsigned_char)(
				(vector_unsigned_short)pkt_mb4 - crc_adjust);
		pkt_mb3 = (vector_unsigned_char)(
				(vector_unsigned_short)pkt_mb3 - crc_adjust);
		//pkt_mb4 = (vector_unsigned_char)vec_sub(
		//		(vector_unsigned_short)pkt_mb4, crc_adjust);
		//pkt_mb3 = (vector_unsigned_char)vec_sub(
		//		(vector_unsigned_short)pkt_mb3, crc_adjust);

		/* pkt 1,2 shift the pktlen field to be 16-bit aligned*/
		const vector_unsigned_int len1 = vec_sll(
			vec_xld2(0, (unsigned int *)&descs[1]),
			(vector_unsigned_int){0, 0, 0, PKTLEN_SHIFT});
		const vector_unsigned_int len0 = vec_sll(
			vec_xld2(0, (unsigned int *)&descs[0]),
			(vector_unsigned_int){0, 0, 0, PKTLEN_SHIFT});
		...
		staterr = (vector_unsigned_short)vec_mergeh(
		...
		vec_xstd2(pkt_mb3, 0, (unsigned char *)&rx_pkts[pos + 2]
                        ->rx_descriptor_fields1);
		/*
		vec_store_len(pkt_mb4, (unsigned char *)&rx_pkts[pos + 3]
                        ->rx_descriptor_fields1, 16);
		vec_store_len(pkt_mb3, (unsigned char *)&rx_pkts[pos + 2]
                        ->rx_descriptor_fields1, 16);
		vec_st(pkt_mb4, 0,
		 (vector_unsigned_char *)&rx_pkts[pos + 3]
			->rx_descriptor_fields1
		);
		vec_st(pkt_mb3, 0,
		 (vector_unsigned_char *)&rx_pkts[pos + 2]
			->rx_descriptor_fields1
		);
		*/

		/* D.2 pkt 1,2 set in_port/nb_seg and remove crc */
		pkt_mb2 = (vector_unsigned_char)(
				(vector_unsigned_short)pkt_mb2 - crc_adjust);
		pkt_mb1 = (vector_unsigned_char)(
				(vector_unsigned_short)pkt_mb1 - crc_adjust);
		//pkt_mb2 = (vector_unsigned_char)vec_sub(
		//		(vector_unsigned_short)pkt_mb2, crc_adjust);
		//pkt_mb1 = (vector_unsigned_char)vec_sub(
		//		(vector_unsigned_short)pkt_mb1,	crc_adjust);
		...
		if (split_packet) {
			...
			vector_unsigned_char eop_bits = vec_and(
				(vector_unsigned_char)vec_nor(staterr, staterr),
				(vector_unsigned_char)eop_check);
		...
		/*
		vec_store_len(pkt_mb2, (unsigned char *)&rx_pkts[pos + 1]
                        ->rx_descriptor_fields1, 16);
		vec_store_len(pkt_mb1, (unsigned char *)&rx_pkts[pos]
                        ->rx_descriptor_fields1, 16);
		vec_st(pkt_mb2, 0,
		 (vector_unsigned_char *)&rx_pkts[pos + 1]
			->rx_descriptor_fields1
		);
		vec_st(pkt_mb1, 0,
		 (vector_unsigned_char *)&rx_pkts[pos]->rx_descriptor_fields1
		);
		*/
